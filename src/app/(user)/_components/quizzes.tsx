...
const ScoreBadge: React.FC<{ percentage: number; attempts: number; size?: 'sm' | 'md' | 'lg' }> = ({
  ...
  return (
    <div className={`inline-flex items-center rounded-full border-2 font-medium shadow-lg ${config.color} ${config.glow} ${sizeClasses[size]} transform transition-all duration-200 hover:scale-105`}>
      <Icon className={`${iconSizes[size]} mr-2`} />
      <span>{config.label}</span>
      {attempts > 1 && <span className="ml-2 opacity-75 text-xs">(#{attempts})</span>}
    </div>
  );
};
...
const QuizDisplay: React.FC<QuizDisplayProps> = ({ quizzes, userId, onScoreUpdate }) => {
    ...
    const loadScores = async () => {
      try {
        ...
        Object.keys(scoresByQuiz).forEach(quizId => {
          ...
          quizScores.sort((a, b) => new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime());
  ...
  if (showResults && currentScore) {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl p-8 shadow-xl text-center">
        ...
        <h2 className="text-4xl font-extrabold text-gray-900 mb-3">Quiz Terminé !</h2>
        <p className="text-xl text-gray-700 mb-6">Vous avez obtenu un score de <span className="font-bold text-blue-600">{currentScore.score}</span> sur <span className="font-bold text-blue-600">{currentScore.totalQuestions}</span>.</p>
        
        <div className="mb-8">
          <CircularProgress percentage={currentScore.percentage} size={150} strokeWidth={10} />
        </div>

        <div className="space-y-3 mb-8">
          <p className="text-lg font-semibold text-gray-800 flex items-center justify-center"><TrendingUp className="w-5 h-5 mr-2 text-green-500" /> Taux de réussite: <span className="text-green-600 ml-2">{currentScore.percentage}%</span></p>
  ...
  if (!selectedQuiz || !currentQuestion) {
    return <div className="text-center py-12 text-gray-600">Chargement du quiz...</div>;
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl p-8 max-w-3xl mx-auto my-8">
      ...
      <div className="mb-8 p-6 bg-blue-50 rounded-xl border border-blue-200">
        <p className="text-xl font-medium text-gray-800">{currentQuestion.content}</p>
      </div>
